...
const StatCard = (stat) => {
  ...
  return (
    <Card className={`"p-4 sm:p-5 "${stat.className}`}>
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <p className={`${stat.className} "text-xs font-medium text-muted-foreground sm:text-sm"`}>
            {stat.title}
          </p>
          <p className={` ${stat.className} "mt-2  font-semibold tracking-tight text-md lg:text-xl"`}>
            {stat.value}
          </p>
          {stat.change && (
